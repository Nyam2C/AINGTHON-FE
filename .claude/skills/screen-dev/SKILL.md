---
name: screen-dev
description: 화면 1장과 사용자 시나리오를 받아 plan → todo → TDD(test → implement) → review 파이프라인으로 React 화면을 개발하는 프론트엔드 개발 오케스트레이터. "화면 만들어줘", "이 시나리오대로 화면 개발", "{화면이름} 페이지 추가", "로그인/회원가입/상세/목록 화면 만들어", 시나리오와 함께 화면 명세를 던질 때 반드시 사용. 후속 작업("이 화면 다시", "{화면}만 수정", "review 결과 반영해서 다시")에도 사용.
---

# screen-dev — 화면 개발 오케스트레이터

화면 1개 + 시나리오를 받아 plan(설계) → todo(코드 사전 명세) → test(실패 테스트) → implement(통과시키기) → review(리팩토링) 5단계로 자동 진행한다. 사용자에게는 review 보고서만 최종 출력한다.
...
검증: `_workspace/{screen_name}/02_todo.md` 존재 + "행동 명세" 섹션이 비어있지 않은지 확인.

## Phase 3: 테스트 작성 (test-author)

```
Agent({
  description: "실패 테스트 작성",
  subagent_type: "general-purpose",
  model: "opus",
  prompt: `
    .claude/agents/test-author.md 의 역할에 따라 작업하라.
    입력: _workspace/{screen_name}/02_todo.md (특히 행동 명세)
    출력:
    - 테스트 파일들 (대상 옆 *.test.tsx / *.test.ts)
    - _workspace/{screen_name}/03_test_summary.md

    먼저 다음을 읽어라:
    - .claude/agents/test-author.md
    - .claude/skills/screen-dev/references/test-patterns.md
    - _workspace/{screen_name}/02_todo.md

    package.json, vite.config.ts, src/test/setup.ts(있으면) 의 현재 상태를 확인하고,
    테스트 셋업이 없으면 셋업 변경을 03_test_summary.md에 기록하라.
  `
})
```

검증: 03_test_summary.md 존재 + 매핑 표가 비어있지 않은지 확인.

## Phase 4: 구현 (implementer)

```
Agent({
  description: "구현 + 테스트 통과",
  subagent_type: "general-purpose",
  model: "opus",
  prompt: `
    ...
    출력:
    - src/ 하위 구현 파일들
    - _workspace/{screen_name}/04_implementation_summary.md
    ...
    셋업 변경(패키지 설치 등)이 03_test_summary.md에 명시되어 있으면 먼저 처리하라.
    pnpm test --run 이 모두 통과할 때까지 반복하라. 단 5회 시도 후에도 진척이 없으면
    멈추고 04_implementation_summary.md "미해결" 섹션에 사유를 기록하라.
  `
})
```

검증: `pnpm test --run` 결과 — 04_implementation_summary.md에 "✓ 통과" 표시가 있는지 확인. 실패가 있으면 Phase 4를 1회 재실행. 그래도 실패면 사용자에게 보고하고 review로 진행 (review가 보류로 처리).
...
Agent({
  ...
  prompt: `
    ...
    출력:
    - 리팩토링이 적용된 src/ 파일들
    - _workspace/{screen_name}/05_review.md
    ...
    각 리팩토링 적용 후 pnpm test --run 으로 회귀 확인.
    실패하면 즉시 되돌리고 "보류"로 분류.
    최종 검증: pnpm test --run, pnpm typecheck, pnpm lint
  `
})
```
...
Phase 5 완료 후 사용자에게 보여줄 것:

1. `_workspace/{screen_name}/05_review.md` 의 핵심 섹션 (요약, 적용한 리팩토링, 보류 항목, 검증 결과, 변경 요약, 다음 단계 제안)을 본문으로 출력
2. 화면 이름과 추가된 라우트 경로
3. 작성된 파일 수와 테스트 통과 여부
...
| 3 test | 02_todo.md (행동 명세) | 테스트 파일들 + 03_test_summary.md | 4 |
| 4 implement | 01, 02, 03, 테스트 파일 | src/ 파일들 + 04_implementation_summary.md | 5 |
| 5 review | 04 + src/ 파일들 | 갱신된 src/ + 05_review.md | 사용자 |
...
| Phase 3 셋업 변경 필요 | 03_test_summary.md에 기록 → Phase 4가 처리 |
| Phase 4 패키지 설치 권한 거부 | 사용자에게 수동 설치 명령 안내 후 중단 |
| Phase 4 테스트 실패 지속 (5회 후) | 04_implementation_summary.md "미해결" 기록, Phase 5로 진행 (review가 처리) |
| Phase 5 리팩토링 후 테스트 깨짐 | 즉시 되돌리고 "보류"로 분류 |
| Phase 5 typecheck/lint 실패 | 05_review.md에 기록, 사용자에게 보고 |
...
예: "review 다시" → Phase 5만 재실행 (1~4의 산출물 그대로 사용)
예: "todo부터 다시" → Phase 2~5 재실행
...
→ Phase 1~5 자동 진행
→ 최종 산출물: src/screens/LoginScreen.tsx, src/components/auth/LoginForm.tsx, src/hooks/useLoginMutation.ts, src/api/auth.ts, src/types/auth.ts, 테스트 파일들, router 수정
→ 사용자에게 05_review.md 요약 출력

### 에러 흐름 (테스트 셋업 부재)
첫 화면이라 vitest 미설치 상태:

→ Phase 3 test-author가 03_test_summary.md에 "vitest 등 미설치, 추가 필요" 기록
→ Phase 4 implementer가 처음에 `pnpm add -D vitest @testing-library/react ...` 실행
→ 권한 승인 시 진행, 거부 시 사용자에게 수동 명령 안내 후 중단
